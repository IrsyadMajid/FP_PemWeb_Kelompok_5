<body class="bg-gray-100">

    

  <!-- SIDEBAR untuk Desktop -->
  <nav class="fixed top-0 bottom-0 left-0 w-[300px] bg-blue-600 p-4 flex-col items-center justify-center hidden lg:flex z-10">
    <div class="flex flex-col gap-4 w-full max-w-[250px]">
      <a href="../dashboard/index.php" class="flex items-center gap-3 p-3 rounded-md bg-blue-700 hover:bg-blue-500 text-white">
        <i class="bi bi-clipboard2-data-fill text-xl"></i>
        <span class="text-lg">Dashboard</span>
      </a>
      <a href="../hasil penjualan/index.php" class="flex items-center gap-3 p-3 rounded-md bg-blue-700 hover:bg-blue-500 text-white">
        <i class="bi bi-tablet-landscape-fill text-xl"></i>
        <span class="text-lg">Hasil Penjualan</span>
      </a>
      <a href="../komisi pegawai/index.php" class="flex items-center gap-3 p-3 rounded-md bg-blue-700 hover:bg-blue-500 text-white">
        <i class="bi bi-cash-coin"></i>
        <span class="text-lg">Komisi Pegawai</span>
      </a>
      <a href="../stok/index.php" class="flex items-center gap-3 p-3 rounded-md bg-blue-700 hover:bg-blue-500 text-white">
        <i class="bi bi-box-seam-fill text-xl"></i>
        <span class="text-lg">Stok</span>
      </a>
      <a href="../pegawai/index.php" class="flex items-center gap-3 p-3 rounded-md bg-blue-700 hover:bg-blue-500 text-white">
        <i class="bi bi-people-fill"></i>
        <span class="text-lg">Pegawai</span>
      </a>
    </div>
  </nav>

  <!-- BOTTOM BAR untuk Mobile -->
  <nav class="fixed bottom-0 left-0 right-0 bg-blue-600 p-2 flex justify-around text-white lg:hidden">
    <a href="../dashboard/index.php" class="flex flex-col items-center">
      <i class="bi bi-clipboard2-data-fill text-xl"></i>
      <span class="text-xs">Dashboard</span>
    </a>
    <a href="../hasil penjualan/index.php" class="flex flex-col items-center">
      <i class="bi bi-tablet-landscape text-xl"></i>
      <span class="text-xs">Penjualan</span>
    </a>
    <a href="../komisi pegawai/index.php" class="flex flex-col items-center">
      <i class="bi bi-cash-coin"></i>
      <span class="text-xs">Komisi</span>
    </a>
    <a href="../stok/index.php" class="flex flex-col items-center">
      <i class="bi bi-box-seam-fill text-xl"></i>
      <span class="text-xs">Stok</span>
    </a>
    <a href="../pegawai/index.php" class="flex flex-col items-center">
      <i class="bi bi-people-fill"></i>
      <span class="text-xs">Pegawai</span>
    </a>
  </nav>

</body>
